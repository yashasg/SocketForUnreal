...
namespace Boggle
{
    ...
    public class BoggleBoard
    {
        ...
        public BoggleBoard()
        {
            // Shuffle the cubes
            Random r = new Random();
            for (int i = this.cubes.Length - 1; i >= 0; i--)
            {
                int j = r.Next(i + 1);
                string temp = this.cubes[i];
            ...
            {
                letters += this.cubes[i][r.Next(6)];
            }

            // Make the board
            this.MakeBoard(letters);
        }
        ...
        public bool CanBeFormed(string word)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    if (this.CanBeFormed(word, i, j, visited))
                    {
                        return true;
                    }
                }
        ...
        private bool CanBeFormed(string word, int i, int j, bool[,] visited)
        {
            ...
            if (this.CanBeFormed(rest, i - 1, j - 1, visited))
            {
                return true;
            }
            if (this.CanBeFormed(rest, i - 1, j, visited))
            {
                return true;
            }
            if (this.CanBeFormed(rest, i - 1, j + 1, visited))
            ...
            if (this.CanBeFormed(rest, i, j - 1, visited))
            {
                return true;
            }
            if (this.CanBeFormed(rest, i, j + 1, visited))
